...
class Ui_DockWidget(object):
    def setupUi(self, DockWidget, setRowCount=3, setColumnCount=3):
    ...
    def retranslateUi(self, DockWidget):
        DockWidget.setWindowTitle(_translate("DockWidget", "Volume calculator", None))
    ...
    def put_in_table(self, list_name, list_volume):
        ...
        self.data.append(["Oject name","V[mm3]","V[m3]"])


        for i in range(0, n):
                self.table.setItem(i+1, 0, QtGui.QTableWidgetItem(list_name[i]))
                self.table.setItem(i+1, 1, QtGui.QTableWidgetItem("%.2f" % list_volume[i]))
                self.table.setItem(i+1, 2, QtGui.QTableWidgetItem("%.2f" % (list_volume[i]*1e-9)))
                self.data.append([list_name[i],list_volume[i],list_volume[i]*1e-9])

        self.table.setItem(n+1, 0, QtGui.QTableWidgetItem("sum"))
        self.table.setItem(n+1, 1, QtGui.QTableWidgetItem("%.2f" % volume_sum))
        self.table.setItem(n+1, 2, QtGui.QTableWidgetItem("%.2f" % (volume_sum*1e-9)))
        self.data.append(["sum","%.2f" % volume_sum,"%.2f" % (volume_sum*1e-9)])
...
class Volume:
  ...
  def GetResources(self):
    ...
    icon = os.path.join( iconPath , 'Volume.png')
    return {'Pixmap'  : icon , # the name of a svg file available in the resources
            'MenuText': "Volume calculator" ,
            'Accel' : "Shift+V",
            'ToolTip' : "Calc volume"}
